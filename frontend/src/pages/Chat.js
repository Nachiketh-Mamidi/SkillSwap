  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        ...
        // Automatically select the first chat if no chatId is provided and chats exist
        if (!selectedChatId && res.data.length > 0) {
          setSelectedChatId(res.data[0].id);
        }

        // If no chats exist, stop loading
              ...
              chats.map((chat) => {
                ...
                return (
                  <ListItem
                    ...
                    sx={{
                      backgroundColor: chat.id === selectedChatId ? '#e3f2fd' : 'inherit',
                      '&:hover': {
                        backgroundColor: '#f1f1f1',
                      },
                      borderLeft: chat.id === selectedChatId ? '4px solid #007bff' : 'none',
                    }}
                  >
                    <ListItemText
              ...
              <Box p={2} bgcolor="#f1f1f1" borderBottom="1px solid #ccc">
                ...
                {skillsToTeach.length > 0 ? (
                  skillsToTeach.map((skill, index) => (
                    <Chip key={index} label={skill} color="success" sx={{ mr: 1, mb: 1 }} />
                  ))
                ) : (
                ...
                {skillsToLearn.length > 0 ? (
                  skillsToLearn.map((skill, index) => (
                    <Chip key={index} label={skill} color="primary" sx={{ mr: 1, mb: 1 }} />
                  ))
                ) : (
